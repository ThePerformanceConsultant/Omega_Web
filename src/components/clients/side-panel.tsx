...
export function SidePanel({ title, onClose, children }: SidePanelProps) {
  return (
    <div className="h-full min-w-0 bg-transparent flex flex-col self-stretch">
      {/* Header */}
      <div className="flex items-center justify-between px-8 py-5 border-b border-black/10 shrink-0 bg-white z-10">
      ...
      {/* Content */}
      <div className="flex-1 min-h-0 overflow-hidden px-8 py-7">{children}</div>
    </div>
  );
}
